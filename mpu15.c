...
/* CONFIG  - //TODO: use a device struct */
...
/* Configuration handling functions */
  void 	mpu_cfg_set_dlpf(int *fd, int dlf_config); //TODO
...
int main( int argc, char **argv )
{
	...
	unsigned int arg_sampling_rate = 0;
	unsigned int arg_accel_range = 0;
	unsigned int arg_gyro_range = 0;
	unsigned int arg_sampling_flag = 0;
	unsigned int arg_accel_flag = 0;
	unsigned int arg_gyro_flag = 0;
	unsigned int arg_thermo_flag = 0;
	unsigned int arg_calibration_flag = 0;
	unsigned int arg_calibration_time = 0;
	unsigned int arg_dump_flag = 0;
	...
	while ( (c = getopt(argc, argv, "c:d:s:a:g:t")) != -1)
	{
		...
		{
		case 'c': // Calibrate
			arg_calibration_flag = 1;
			if ((sscanf(optarg, "%d", &arg_calibration_time)) == EOF) {
				fprintf(stderr,"error writing to arg_calibration_time");
				exit(1);
			}
			break;
		case 'd': // dump
			arg_dump_flag = 1;
			if ((sscanf(optarg, "%s", &arg_dump_filename)) == EOF) {
				fprintf(stderr,"error writing to arg_dump_filename");
				exit(1);
			}
		case 's': // Sampling Frequency TODO: min max values
			arg_sampling_flag = 1;
			if ( (sscanf(optarg, "%d", &arg_sampling_rate)) == EOF) {
				fprintf(stderr,"error writing to arg_sampling_rate");
				exit(1);
			}
			/*
			switch(arg_sampling_rate)
			{
			case 50 : break;
			case 100 : break;
			case 200 : break;
		...
		case 'a': // Accel-range
			arg_accel_flag = 1;
			if ((sscanf(optarg, "%d", &arg_accel_range)) == EOF) {
				fprintf(stderr, "error writing to arg_accel_range");
				exit(1);
			}
			switch (arg_accel_range)
			{
			case 2 : break;
			case 4 : break;
			case 8 : break;
		...
		case 'g': //Gyro-range
			arg_gyro_flag = 1;
			if ((sscanf(optarg, "%d", &arg_gyro_range)) == EOF) {
				fprintf(stderr, "Error writing to arg_gyro_range");
				exit(1);
			}
			switch (arg_gyro_range)
			{
			case 250 : break;
			case 500 : break;
			case 1000 : break;
	...
	if( arg_sampling_flag == 1 ) {
		mpu_cfg_set_samplerate(&mpu6050,arg_sampling_rate);
	}
	if( arg_gyro_flag == 1 ) {
		mpu_cfg_set_gyro(&mpu6050,arg_gyro_range);
	}
	if( arg_accel_flag == 1 ) {
		mpu_cfg_set_accel(&mpu6050,arg_accel_range);
	}
	if( arg_thermo_flag ==1 ) {
		mpu_cfg_set_thermo(&mpu6050);
	}
	mpu_ctl_start(&mpu6050);
	if( arg_calibration_flag ==1 ) {
 		mpu_get_calibration(&mpu6050,arg_calibration_time);
	}
	...
	if( arg_dump_flag ==1 ) {
 		mpu_dumpreg(&mpu6050,arg_dump_filename);
	}
	mpu_ctl_flush_fifo(&mpu6050);
